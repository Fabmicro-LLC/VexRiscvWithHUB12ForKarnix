...
case class MacRxBuffer(pushCd : ClockDomain,
  ...
  val io = new Bundle {
    ...
    val pop = new Bundle{
      ...
      val stats = new Bundle {
    	...
    	val length = out UInt(lengthWidth bits)
	val full = out Bool() 
      }
    }
  }
  ...
  val push = pushCd on new Area{
    ...
    val length_out = Reg(UInt(lengthWidth bits)) init(0)
  ...
  val pop = popCd on new Area{
    ...
    val stats = new Area{
      ...
      val full = PulseCCByToggle(
        input = push.full,
        clockIn = pushCd,
        clockOut = popCd
      )
      io.pop.stats.full := full 

      val length  = BufferCC(push.length_out)
      io.pop.stats.length := length
