...
case class MacEthCtrl(p : MacEthParameter) extends Bundle{
  val rx = new Bundle {
    ...
    val stats = new Bundle {
      ...
      val full = out Bool()
      val length = out UInt(log2Up(p.rxBufferByteSize*8) bits);
    }
  }
  val tx = new Bundle {
  ...
  def driveFrom(bus: BusSlaveFactory) = new Area{
    ...
    bus.read(rx.stats.length, 0x30, 0)
...
case class MacEth(p : MacEthParameter,
  ...
  val rxBackend = new Area{
    ...
    io.ctrl.rx.stats.full <> rxFrontend.buffer.io.pop.stats.full
    io.ctrl.rx.stats.length <> rxFrontend.buffer.io.pop.stats.length
  }
